...
const fetchAll = async () => {
  ...
  if (dataCommunity) {
    ...
    for (let i = 0; i < maxLengthCommunity; i++) {
      ...
      a.setAttribute(
        "href",
        `/community/${dataCommunity[i].id}`
      );

      // Upload images
      const div = document.createElement("div");
      div.classList.add("community_card");
      const sourceString = `/uploads/${dataCommunity[i].communityImage}`;
  ...
  if (dataProject) {
    for (let i = 0; i < maxLengthProject; i++) {
      ...
      a.setAttribute(
        "href",
        `/project/${dataProject[i].id}`
      );
      // Upload images
      const div = document.createElement("div");
      div.classList.add("community_card");
      const sourceString = `/uploads/${dataProject[i].projectCoverImage}`;
  ...
  if (dataEvent) {
    for (let i = 0; i < maxLengthEvent; i++) {
      ...
      a.setAttribute("href", `/event/${dataEvent[i].id}`);
      // Upload images
      const div = document.createElement("div");
      div.classList.add("community_card");
      const sourceString = `/uploads/${dataEvent[i].eventImage}`;
